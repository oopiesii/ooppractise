...
void Program::run() {
    string input;
    std::string command;
    std::set<std::string> commands = {"add", "draw", "list", "colors", "figures", "exit", "clear","remove", "save", "load", "select"};
    ...
    cout << "Commands are 1. add, 2. draw, 3. list, 4. colors, 5. figures, 6. exit, 7. clear, 8. save, 9. load, 10. remove, 11. select " << endl;
        ...
        try {
            ...
            if (command == "add") {
                ...
                if (params[1] == "circle") {
                    ...
                    Circle* c = new Circle(canvas.allfiguresoncanvas.size() + 1,params[2], params[3], stoi(params[4]), stoi(params[5]), stoi(params[6]));
                    c->draw(canvas);
                    ...
                    Line* l = new Line(canvas.allfiguresoncanvas.size() + 1, params[2], params[3], stoi(params[4]), stoi(params[5]), stoi(params[6]));
                    l->draw(canvas);
                    canvas.allfiguresoncanvas.push_back({params[2], params[1]});
                    ...
                    Triangle* t = new Triangle(canvas.allfiguresoncanvas.size() + 1,params[2], params[3], stoi(params[4]), stoi(params[5]), stoi(params[6]));
                    t->draw(canvas);
                    canvas.allfiguresoncanvas.push_back({params[2], params[1]});
                    ...
                    Rectangle* r = new Rectangle(canvas.allfiguresoncanvas.size() + 1, params[2], params[3], stoi(params[4]), stoi(params[5]), stoi(params[6]), stoi(params[7]));
                    r->draw(canvas);
                    canvas.allfiguresoncanvas.push_back({params[2], params[1]});
